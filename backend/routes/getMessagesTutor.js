...
router.post( '/getMessages', auth, roleCheck(['Tutor']), (req, res) =>{
    ...
    mysql.query(queryy, [uuid], (err, resul)=> {
        if (err) {
            ...
            const query ="SELECT TextID, IdTutor, IdLearner, message, messageTime, Sender, learner.id, firstname, lastname, pfp from convo, learner where convo.IdLearner=learner.id and ?=IdTutor and ?=IdLearner ";
