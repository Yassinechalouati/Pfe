...
router.post( '/getMessages', auth, roleCheck(['Learner']), (req, res) =>{
    ...
    mysql.query(queryy, [uuid], (err, resul)=> {
        if (err) {
            ...
            const query ="SELECT TextID, IdTutor, IdLearner, message, messageTime, Sender, firstname, lastname, tutor.id, pfp from convo, tutor where convo.IdTutor=tutor.id and ?=IdLearner and ?=IdTutor ";
