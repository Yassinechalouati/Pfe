...
export default function Card() {
    ...
    return(
        <form onSubmit={handleNext} className="bg-white overflow-y-auto px-10 py-4 w-[97%] h-[95%] sm:w-[80%] sm:h-[93%] md:w-[60%] md:h-[93%] lg:w-[60%] lg:h-[93%] xl:w-[60%] xl:h-[93%] space-y-4 rounded-3xl flex flex-col items-center">
            ...
            <div className={`flex ${step === 0? "justify-end": "justify-between"} w-full flex-grow items-center`}>
                <button type="submit" onClick={handlePrevious} className={`text-button bg-lightbutton border-button  border space-x-2 ${step === 0? "hidden": ""} sm:space-x-2 md:space-x-2 lg:space-x-2 xl:space-x-2 flex justify-center items-center sm:w-[15%] md:w-[15%] lg:w-[15%] xl:w-[15%] w-[20%] self-end h-10 text-center font-semibold text-lg px-4 py-2 rounded-full cursor-pointer hover:shadow`}>
