...
import { setListOfWorkExperienceVisibility } from "../../state/slices/listSlice"
...
function WorkExperience(props) {
    ...
    const handleAddWorkExperience = () => {
        dispatch(addWorkExperience({id: listOfWorkExperience.length, title:'', description:'', tag: ''}))
    }
    ...
    const content = [
        <div key="0" className="h-full w-full flex flex-col justify-between overflow-y-auto space-y-5">
            {listOfWorkExperience.map((field, index) => {
                return <Work key={index} index={field.id} ></Work>
            })}
        </div>,
    ...
    return (
        <>
            ...
            <div className="w-[80%] md:w-[50%] h-auto flex-col m-auto flex space-y-2 p-[13px] shadow bg-lightg rounded-xl">
                ...
                <button onClick={handleWorkExperienceModal} className=" bg-button2 text-sm text-white w-44 h-10 rounded-lg">
                    ADD WORK EXPERIENCE
                </button>
            </div>
        </>
