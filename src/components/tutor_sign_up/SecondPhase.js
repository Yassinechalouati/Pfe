...
import {setDescription, setIntroductionVideo, setError, setDisplayableImage} from '../../state/slices/tutorSlice'
...
function ProfilePictureUploader() {
            ...
            try {
                ...
                if(response.data.message){
                    ...
                    reader.onerror = error => {
                        dispatch(setError('Cannot load image!'))
                        console.log("Error", error)
                    }
                }
                else {
                    dispatch(setError('No face Detected'))
                }
            } catch (err) {
    ...
    const handleVideoChange= (event) => {
        ...
        if (event.target.files.length > 0) {
            ...
            //the below part not even working 
        ...
        <div className="w-full h-[80%] space-y-2 py-3 overflow-y-auto"> 
            <div className="rounded-xl flex flex-col h-[46%] m-auto w-[80%] md:w-[50%] relative">
                ...
                <div className="bg-lightg w-full p-[13px] h-[50%] rounded-b-xl flex flex-col justify-end">
                    ...
                    <span className={`text-sm ${ tutorData.displayableImage? 'text-green-600 font-bold': 'text-darkg'}`}>{
